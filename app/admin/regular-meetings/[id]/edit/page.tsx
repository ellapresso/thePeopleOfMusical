...
import { getRegularMeetingScheduleById, updateRegularMeetingSchedule, getAllMembers } from '@/lib/adminApi';
import { useAdmin } from '@/contexts/AdminContext';
import styles from '../../../admin.module.css';

export default function EditRegularMeetingPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const schedule = await getRegularMeetingScheduleById(id);
      ...
      setFormData({
        ...
        managerId: schedule.managerId ? schedule.managerId.toString() : '',
      });

      // 담당자 정보 설정
      if (schedule.managerId && schedule.manager) {
        setSelectedManager(schedule.manager);
        setManagerSearchQuery(schedule.manager.name);
      }
    } catch (err: any) {
      setError(err.message || '정기 모임 정보를 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateRegularMeetingSchedule(id, {
      ...
      setError(err.message || '정기 모임 수정에 실패했습니다.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className={styles.container}>
      <div className={styles.content}>
        <div className={styles.header}>
          <div>
            <h1 className={styles.headerTitle}>정기 모임 수정</h1>
            <p className={styles.headerSubtitle}>정기 모임 정보를 수정합니다</p>
          </div>
        ...
        <div style={{
          ...
          {success && (
            <div style={{
              ...
              정기 모임이 성공적으로 수정되었습니다. 목록 페이지로 이동합니다...
            </div>
          )}
          {error && (
            <div style={{
